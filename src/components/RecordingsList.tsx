...
const RecordingsList = () => {
  ...
  const { recordings, recordingsLoaded } = useLoadRecordings(call);
  const { user, isLoaded: userLoaded } = useUser();

  if (userLoaded && !user) {
    return (
      <p className="text-center">You must logged in to view recordings.</p>
    );
  }

  if (recordingsLoaded) return <Loader2 className="mx-auto animate-spin" />;
